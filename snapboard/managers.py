...
class PostManager(models.Manager):
    def get_queryset(self):
        ...
        return super(PostManager, self).get_queryset().extra(
    ...
    def posts_for_thread(self, thread_id, user):
        ...
        qs = self.get_queryset().filter(thread__id=thread_id).select_related("user__userprofile").distinct()
        ...
        #print "First", qs[0].user.userprofile.rating_score.average
        return qs
...
class ThreadManager(models.Manager):
    def get_queryset(self):
        ...
        return super(ThreadManager, self).get_queryset().extra(
            select = {
                'post_count': extra_post_count,
                'starter': extra_starter,
                'starter_id': extra_starter_id,
                'rating': extra_rating,
    ...
    def get_favorites(self, user):
        ...
        return self.get_queryset().filter(pk__in=[x.thread_id for x in wl])
    ...
    def get_private(self, user):
        ...
        return self.get_queryset().filter(pk__in=thread_ids)

    def get_category(self, cat_id):
        return self.get_queryset().filter(category__id=cat_id)
...
class CategoryManager(models.Manager):
    def get_queryset(self):
        thread_count = """
            ...
            """
        return super(CategoryManager, self).get_queryset().extra(
